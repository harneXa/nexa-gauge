...
RELEVANCE_FILE = Path(__file__).parent.parent.parent.parent / "data" / "relevance.json"
OUTPUT_FILE = Path(__file__).parent / "relevance_metrics.json"
...
def main():
    ...
    for entry in records:
        ...
        question_obj = inputs.get("question") or {}
        question_text = question_obj.get("text", "")
        question_tokens = question_obj.get("tokens")

        relevance_metrics = entry.get("relevance_metrics", {})
        metrics = relevance_metrics.get("metrics", [])
        cost_entry = relevance_metrics.get("cost", {})
        ...
        print(f"text_tokens={gen_tokens}")
